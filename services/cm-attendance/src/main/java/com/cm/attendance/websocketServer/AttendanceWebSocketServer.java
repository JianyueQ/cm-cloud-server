package com.cm.attendance.websocketServer;
...
public class AttendanceWebSocketServer {

    // 静态变量，用来记录当前在线连接数
    private static int onlineCount = 0;

    // concurrent包的线程安全Set，用来存放每个客户端对应的MyWebSocket对象
    private static CopyOnWriteArraySet<AttendanceWebSocketServer> webSocketSet = new CopyOnWriteArraySet<>();

    // 用于记录用户ID与该WebSocket连接的映射关系
    private static ConcurrentHashMap<Long, AttendanceWebSocketServer> webSocketMap = new ConcurrentHashMap<>();
    ...
    public static void sendMessageToUser(Long userId, String message){
        AttendanceWebSocketServer webSocket = webSocketMap.get(userId);
        if (webSocket != null){
    ...
    public static synchronized void subOnlineCount() {
        AttendanceWebSocketServer.onlineCount--;
    }
