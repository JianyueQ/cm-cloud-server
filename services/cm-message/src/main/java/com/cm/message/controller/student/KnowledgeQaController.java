 ...
 */
@Slf4j
@RestController("studentKnowledgeQaController")
...
public class KnowledgeQaController {
    ...
    public SseEmitter askQuestionStream(@RequestBody Map<String, String> request){
        ...
        // 需要立即执行耗时任务并保持连接活跃时（如 SSE 流式传输)使用CompletableFuture
        CompletableFuture.runAsync(() -> {
            try {
                Stream<String> responseStream = knowledgeQaService.answerQuestionStream(request, currentId);
